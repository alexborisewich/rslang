...
const checkArrLength = (objectGames:Game) =>{
  if(objectGames.words.length) {
...
export const onClickHandlerGame = (answer:boolean) => {
  const correctAnswer  = (idWord === idTranslate);
  playSound(answer === correctAnswer, mute);
  if (answer === correctAnswer) {
    ...
    score = score + Math.min(series * 20, 80);
...
backToNewGame!.addEventListener('click', function(){
  indexesWord = getRandomArrId(WORDS_GAME.length);
  idWord = indexesWord.pop();
  idTranslate = getRandomBoolean() ? idWord : getRandomInteger(19);
  gameStatus = true;
